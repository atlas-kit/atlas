...
void Creature::removeCondition(ConditionType_t type, bool force /* = false*/)
{
	...
	while (it != conditions.end()) {
		...
		auto conditionHolder = std::move(*it);
		it = conditions.erase(it);

		conditionHolder->endCondition(asCreature());
...
{
	auto it = std::ranges::find_if(conditions, [condition](const auto& storedCondition) {
		return storedCondition.get() == condition;
	});
	...
	ConditionType_t type = conditionHolder->getType();
	conditionHolder->endCondition(asCreature());
	onEndCondition(type);
}
...
void Creature::executeConditions(uint32_t interval)
{
	...
	for (Condition* condition : tempConditions) {
		...
		if (!condition->executeCondition(asCreature(), interval)) {
			it = std::ranges::find_if(conditions, [condition](const auto& storedCondition) {
				return storedCondition.get() == condition;
			});
			if (it != conditions.end()) {
				auto conditionHolder = std::move(*it);
