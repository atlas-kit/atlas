...
void onFightModeChanged(const std::shared_ptr<Player>& player, FightMode_t stance, bool chase, bool secure)
{
	...
	const auto env = tfs::lua::getScriptEnv();
	env->setScriptId(playerHandlers.onFightModeChanged, &tfs::events::getScriptInterface());

	const auto L = tfs::events::getScriptInterface().getLuaState();
	tfs::events::getScriptInterface().pushFunction(playerHandlers.onFightModeChanged);

	tfs::lua::pushThing(L, player);
	tfs::lua::pushNumber(L, static_cast<uint16_t>(stance));
