...
void Game::start(ServiceManager* manager)
{
	...
	g_appLoop.schedule(createDelayedAppLoopEvent(EVENT_CREATURE_THINK_INTERVAL, [this]() { checkCreatures(0); }));
	g_appLoop.schedule(
	    createDelayedAppLoopEvent(getNumber(ConfigManager::PATHFINDING_INTERVAL), [this]() { updateCreaturesPath(0); }));
	g_appLoop.schedule(createDelayedAppLoopEvent(EVENT_DECAYINTERVAL, [this]() { checkDecay(); }));
}
...
void Game::setGameState(GameState_t newState)
{
	...
	switch (newState) {
		...
		case GAME_STATE_SHUTDOWN: {
			...
			g_appLoop.enqueue([this]() { shutdown(); });
                           ...
                           const Position& toPos, uint8_t count)
{
	...
	if (const auto& movingCreature = thing->asCreature()) {
		...
		if (movingCreature->getPosition().isInRange(player->getPosition(), 1, 1, 0)) {
			auto task = createDelayedAppLoopEvent(
			...
			player->setNextActionEvent(std::move(task));
                              ...
                              const Position& movingCreatureOrigPos, const std::shared_ptr<Tile>& toTile)
{
	if (!player->canDoAction()) {
		...
		auto task =
		    createDelayedAppLoopEvent(delay, [=, this, playerID = player->getID(), movingCreatureID = movingCreature->getID(),
	...
	player->setNextActionEvent(nullptr);

	if (!movingCreatureOrigPos.isInRange(player->getPosition(), 1, 1, 0)) {
		// need to walk to the creature first before moving it
		std::vector<Direction> listDir;
		if (player->getPathTo(movingCreatureOrigPos, listDir, 0, 1, true, true)) {
			g_appLoop.enqueue([this, playerID = player->getID(), listDir = std::move(listDir)]() {
				playerAutoWalk(playerID, listDir);
			});
			auto task = createDelayedAppLoopEvent(RANGE_MOVE_CREATURE_INTERVAL, [=, this, playerID = player->getID(),
			                                                               movingCreatureID = movingCreature->getID(),
			                                                               toPos = toTile->getPosition()] {
				playerMoveCreatureByID(playerID, movingCreatureID, movingCreatureOrigPos, toPos);
			});
			player->setNextWalkActionEvent(std::move(task));
		} else {
                          ...
                          std::shared_ptr<Thing> toThing)
{
	if (!player->canDoAction()) {
		uint32_t delay = player->getNextActionTime();
		auto task = createDelayedAppLoopEvent(delay, [=, this, playerID = player->getID()]() {
	...
	if (!playerPos.isInRange(mapFromPos, 1, 1)) {
		...
		if (player->getPathTo(item->getPosition(), listDir, 0, 1, true, true)) {
			...
			auto task = createDelayedAppLoopEvent(RANGE_MOVE_ITEM_INTERVAL, [=, this, playerID = player->getID()]() {
				playerMoveItemByPlayerID(playerID, fromPos, spriteId, fromStackPos, toPos, count);
			});
			player->setNextWalkActionEvent(std::move(task));
		} else {
	...
	if (item->isHangable() && toThingTile->hasFlag(TILESTATE_SUPPORTS_HANGABLE)) {
		...
		if (!playerPos.isInRange(mapToPos, 1, 1, 0)) {
			...
			if (player->getPathTo(walkPos, listDir, 0, 0, true, true)) {
				...
				auto task = createDelayedAppLoopEvent(RANGE_MOVE_ITEM_INTERVAL, [this, playerID = player->getID(), itemPos,
...
void Game::playerMove(uint32_t playerId, Direction direction)
{
	...
	player->resetIdleTime();
	player->setNextWalkActionEvent(nullptr);
                           ...
                           const Position& toPos, uint8_t toStackPos, uint16_t toSpriteId)
{
	...
	if (ret != RETURNVALUE_NOERROR) {
		if (ret == RETURNVALUE_TOOFARAWAY) {
			...
			if (player->getPathTo(walkToPos, listDir, 0, 1, true, true)) {
				...
				auto task = createDelayedAppLoopEvent(RANGE_USE_ITEM_EX_INTERVAL, [=, this]() {
	...
	if (!player->canDoAction()) {
		uint32_t delay = player->getNextActionTime();
		auto task = createDelayedAppLoopEvent(delay, [=, this]() {
...
void Game::playerUseItem(uint32_t playerId, const Position& pos, uint8_t stackPos, uint8_t index, uint16_t spriteId)
{
	...
	if (!player->canDoAction()) {
		uint32_t delay = player->getNextActionTime();
		auto task =
		    createDelayedAppLoopEvent(delay, [=, this]() { playerUseItem(playerId, pos, stackPos, index, spriteId); });
		player->setNextActionEvent(std::move(task));
		return;
	}
	...
	if (ret != RETURNVALUE_NOERROR) {
		if (ret == RETURNVALUE_TOOFARAWAY) {
			...
			if (player->getPathTo(walkToPos, listDir, 0, 1, true, true)) {
				...
				auto task = createDelayedAppLoopEvent(RANGE_USE_WITH_CREATURE_INTERVAL, [=, this]() {
...
void Game::playerMoveUpContainer(uint32_t playerId, uint8_t cid)
{
	...
	if (!parentContainer) {
		...
		if (it == browseFields.end()) {
			...
			g_appLoop.schedule(createDelayedAppLoopEvent(30000, [this, tile]() { browseFields.erase(tile.get()); }));
		} else {
...
void Game::playerRotateItem(uint32_t playerId, const Position& pos, uint8_t stackPos, const uint16_t spriteId)
{
	...
	if (pos.x != 0xFFFF && !pos.isInRange(player->getPosition(), 1, 1, 0)) {
		...
		if (player->getPathTo(pos, listDir, 0, 1, true, true)) {
			...
			auto task = createDelayedAppLoopEvent(RANGE_ROTATE_ITEM_INTERVAL,
...
void Game::playerBrowseField(uint32_t playerId, const Position& pos)
{
	...
	if (!playerPos.isInRange(pos, 1, 1)) {
		...
		if (player->getPathTo(pos, listDir, 0, 1, true, true)) {
			...
			auto task =
			    createDelayedAppLoopEvent(RANGE_BROWSE_FIELD_INTERVAL, [=, this]() { playerBrowseField(playerId, pos); });
			player->setNextWalkActionEvent(std::move(task));
		} else {
			player->sendCancelMessage(RETURNVALUE_THEREISNOWAY);
		}
...
void Game::playerWrapItem(uint32_t playerId, const Position& position, uint8_t stackPos, const uint16_t spriteId)
{
	...
	if (position.x != 0xFFFF && !position.isInRange(player->getPosition(), 1, 1, 0)) {
		...
		if (player->getPathTo(position, listDir, 0, 1, true, true)) {
			...
			auto task = createDelayedAppLoopEvent(RANGE_WRAP_ITEM_INTERVAL,
                              ...
                              uint16_t spriteId)
{
	...
	if (!tradeItemPosition.isInRange(playerPosition, 1, 1)) {
		...
		if (player->getPathTo(pos, listDir, 0, 1, true, true)) {
			...
			auto task = createDelayedAppLoopEvent(RANGE_REQUEST_TRADE_INTERVAL, [=, this]() {
...
void Game::playerSetAttackedCreature(uint32_t playerId, uint32_t creatureId)
{
	...
	g_appLoop.enqueue([this, id = player->getID()]() { updateCreatureWalk(id); });
}
...
{
	g_appLoop.schedule(createDelayedAppLoopEvent(EVENT_CHECK_CREATURE_INTERVAL,
...
{
	g_appLoop.schedule(createDelayedAppLoopEvent(getNumber(ConfigManager::PATHFINDING_INTERVAL),
