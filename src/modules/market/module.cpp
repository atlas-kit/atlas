...
MarketPurchaseStatistics* g_marketPurchaseStatistics = nullptr;
MarketSaleStatistics* g_marketSaleStatistics = nullptr;
...
void updateStatistics(MarketPurchaseStatistics& purchaseStatistics, MarketSaleStatistics& saleStatistics);
...
void checkExpiredOffers()
{
	...
	g_databaseTasks.addTask(
	    std::format(
	        ...
	        lastExpireDate),
	    [](std::shared_ptr<DBResult> result, bool success) { processExpiredOffers(std::move(result), success); }, true);
}
...
void updateStatistics(MarketPurchaseStatistics& purchaseStatistics, MarketSaleStatistics& saleStatistics)
{
	...
	do {
		...
		if (result->getNumber<uint16_t>("sale") == MARKETACTION_BUY) {
			statistics = &purchaseStatistics.values[result->getNumber<uint16_t>("itemtype")];
		} else {
			statistics = &saleStatistics.values[result->getNumber<uint16_t>("itemtype")];
		}
...
{
	auto it = purchaseStatistics.values.find(itemId);
	if (it == purchaseStatistics.values.end()) {
		return nullptr;
	}
	return &it->second;
}
...
{
	auto it = saleStatistics.values.find(itemId);
	if (it == saleStatistics.values.end()) {
		return nullptr;
	}
	return &it->second;
}

void MarketModule::build(App& app)
{
	app.insert_resource<MarketPurchaseStatistics>();
	app.insert_resource<MarketSaleStatistics>();
	app.insert_resource<ExpiredMarketOffersTimer>();

	app.add_observer<StartupEvent>([](Context& context) {
		auto purchaseStatistics = context.read_resource<MarketPurchaseStatistics>();
		auto saleStatistics = context.read_resource<MarketSaleStatistics>();
		updateStatistics(*purchaseStatistics, *saleStatistics);
	});

	app.add_observer<StartupEvent>([](Context& context) {
		const int32_t checkExpiredMarketOffersEachMinutes =
		    getNumber(ConfigManager::CHECK_EXPIRED_MARKET_OFFERS_EACH_MINUTES);
		if (checkExpiredMarketOffersEachMinutes <= 0) {
			return;
		}

		auto expiredOffersTimer = context.read_resource<ExpiredMarketOffersTimer>();
		expiredOffersTimer->timer = std::make_unique<tfs::application::Timer>(
		    std::chrono::minutes{checkExpiredMarketOffersEachMinutes}, tfs::application::TimerMode::Repeating);
	});

	app.add_system<FixedUpdate>([](Context& context) {
		auto expiredOffersTimer = context.read_resource<ExpiredMarketOffersTimer>();
		if (!expiredOffersTimer->timer) {
			return;
		}

		auto time = context.read_resource<Time<Fixed>>();
		if (expiredOffersTimer->timer->tick(time->elapsed_since_last()).just_finished()) {
			checkExpiredOffers();
		}
	});
	...
	app.add_observer<StartupEvent>([](Context& context) {
		// Legacy bridge for code paths that do not receive an application Context yet.
		g_marketPurchaseStatistics = context.read_resource<MarketPurchaseStatistics>().get();
		g_marketSaleStatistics = context.read_resource<MarketSaleStatistics>().get();
	});
