...
bool startupErrorMessage(const std::string& error)
{
	std::println(BOLDRED "ERROR: {:s}" RESET, error);
	return false;
}

bool mainLoader(ServiceManager* services)
{
	// Main thread
	...
	if (!ConfigManager::load()) {
		return startupErrorMessage("Unable to load " + configFile + "!");
	}
	...
	try {
		...
		return startupErrorMessage(e.what());
	}

	std::cout << ">> Establishing database connection..." << std::flush;

	if (!Database::getInstance().connect()) {
		return startupErrorMessage("Failed to connect to database.");
	}
	...
	if (std::ifstream is{"data/XML/vocations.xml"}; !g_vocations.loadFromXml(is, "data/XML/vocations.xml")) {
		return startupErrorMessage("Unable to load vocations!");
	}

	// load item data
	std::cout << ">> Loading items... ";
	if (!Item::items.loadFromOtb("data/items/items.otb")) {
		return startupErrorMessage("Unable to load items (OTB)!");
	}
	...
	if (!Item::items.loadFromXml()) {
		return startupErrorMessage("Unable to load items (XML)!");
	}

	std::cout << ">> Loading script systems" << std::endl;
	if (!ScriptingManager::getInstance().loadScriptSystems()) {
		return startupErrorMessage("Failed to load script systems");
	}

	std::cout << ">> Loading lua scripts" << std::endl;
	if (!g_scripts->loadScripts("scripts", false, false)) {
		return startupErrorMessage("Failed to load lua scripts");
	}

	std::cout << ">> Loading monsters" << std::endl;
	if (!g_monsters.loadFromXml()) {
		return startupErrorMessage("Unable to load monsters!");
	}

	std::cout << ">> Loading lua monsters" << std::endl;
	if (!g_scripts->loadScripts("monster", false, false)) {
		return startupErrorMessage("Failed to load lua monsters");
	}

	std::cout << ">> Loading outfits" << std::endl;
	if (!Outfits::getInstance().loadFromXml()) {
		return startupErrorMessage("Unable to load outfits!");
	}
	...
	try {
		...
		return startupErrorMessage(std::format("Failed to load map: {:s}", e.what()));
	}
...
class ServerRuntime final : public tfs::application::prelude::Resource
{
public:
	ServiceManager service_manager;
	std::thread service_thread;
	std::atomic_bool service_started{false};
	std::atomic_bool service_finished{false};
};
...
void ServerModule::build(App& app)
{
	app.insert_resource<ServerRuntime>();

	app.add_observer<StartupEvent>([] { std::set_new_handler(badAllocationHandler); });

	app.add_observer<StartupEvent>([](Context& context, StartupEvent& startup_event) {
		auto runtime = context.read_resource<Runtime>();
		auto server_runtime = context.read_resource<ServerRuntime>();
		auto services = std::shared_ptr<ServiceManager>(server_runtime, &server_runtime->service_manager);
		if (!mainLoader(services.get())) {
			startup_event.set_cancelled(true);
		}
	});

	app.add_observer<StartupEvent>([](Context& context) {
		auto runtime = context.read_resource<Runtime>();
		auto server_runtime = context.read_resource<ServerRuntime>();
		if (server_runtime->service_manager.is_running()) {
			std::cout << ">> " << getString(ConfigManager::SERVER_NAME) << " Server Online!" << std::endl << std::endl;
			server_runtime->service_started.store(true, std::memory_order_release);
			server_runtime->service_finished.store(false, std::memory_order_release);
			server_runtime->service_thread = std::thread([server_runtime] {
				server_runtime->service_manager.run();
				server_runtime->service_finished.store(true, std::memory_order_release);
			});
		} else {
	...
	app.add_system<Update>([](Context& context) {
		auto runtime = context.read_resource<Runtime>();
		auto server_runtime = context.read_resource<ServerRuntime>();
		if (server_runtime->service_started.load(std::memory_order_acquire) &&
		    server_runtime->service_finished.load(std::memory_order_acquire)) {
			runtime->stop();
		}
	});

	app.add_observer<ShutdownEvent>([](Context& context) {
		auto server_runtime = context.read_resource<ServerRuntime>();
		if (server_runtime->service_started.load(std::memory_order_acquire) &&
		    !server_runtime->service_finished.load(std::memory_order_acquire)) {
			server_runtime->service_manager.stop();
		}

		if (server_runtime->service_thread.joinable()) {
			server_runtime->service_thread.join();
		}
