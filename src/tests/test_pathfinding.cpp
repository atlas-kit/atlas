...
BOOST_AUTO_TEST_CASE(test_same_position)
{
	...
	BOOST_CHECK(finder.solve(3, 3, fpp, dirList, map, condition, true));
...
BOOST_AUTO_TEST_CASE(test_unreachable)
{
	...
	BOOST_CHECK(!finder.solve(5, 0, fpp, dirList, map, condition, true));
	BOOST_CHECK(dirList.empty());
}
...
BOOST_AUTO_TEST_CASE(test_single_cardinal_step)
{
	...
	FrozenPathingConditionCall condition(Position(1, 0, 7));
	std::vector<Direction> dirList;

	PathFinder finder(0, 0);
	bool ok = finder.solve(1, 0, fpp, dirList, map, condition, true);
	BOOST_CHECK(ok);
...
BOOST_AUTO_TEST_CASE(test_single_diagonal_step)
{
	...
	FrozenPathingConditionCall condition(Position(1, 1, 7));
	std::vector<Direction> dirList;

	PathFinder finder(0, 0);
	bool ok = finder.solve(1, 1, fpp, dirList, map, condition, true);
	BOOST_CHECK(ok);
	BOOST_CHECK_EQUAL(dirList.size(), 1);
}
...
BOOST_AUTO_TEST_CASE(test_linear_path_three_steps)
{
	...
	FrozenPathingConditionCall condition(Position(3, 0, 7));
	std::vector<Direction> dirList;

	PathFinder finder(0, 0);
	bool ok = finder.solve(3, 0, fpp, dirList, map, condition, true);
	BOOST_CHECK(ok);
...
BOOST_AUTO_TEST_CASE(test_cost_avoidance_chooses_cheaper_path)
{
	...
	bool ok = finder.solve(2, 0, fpp, dirList, map, condition, true);
	BOOST_CHECK(ok);
...
BOOST_AUTO_TEST_CASE(test_zero_cost_walkable)
{
	...
	BOOST_CHECK(!finder.solve(1, 0, fpp, dirList, map, condition, true));
	BOOST_CHECK(dirList.empty());
}
...
BOOST_AUTO_TEST_CASE(test_minimal_cost_leads_to_shortest_path)
{
	...
	FrozenPathingConditionCall condition(Position(2, 2, 7));
	std::vector<Direction> dirList;

	PathFinder finder(0, 0);
	bool ok = finder.solve(2, 2, fpp, dirList, map, condition, true);
	BOOST_CHECK(ok);
...
{
	FrozenPathingConditionCall cond(Position(5, 3, 7));
...
BOOST_AUTO_TEST_CASE(test_long_path_within_limits)
{
	...
	bool ok = finder.solve(5, 0, fpp, dirList, map, condition, true);
	BOOST_CHECK(ok);
...
BOOST_AUTO_TEST_CASE(test_solve_same_position_false_condition)
{
	...
	PathFinder finder(0, 0);
	bool ok = finder.solve(0, 0, fpp, dirList, map, condition, true);
